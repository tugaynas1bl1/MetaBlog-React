...
const Search = ({searchTxt, blogs}) => {
    ...
    const { setSearchReload } = useSearch()
    ...
    return (
        <div className={`${isDarkModeActive ?  "bg-gray-800" : "bg-[#ababac]" } xl:w-7xl lg:w-280 md:w-120 w-80 z-40 flex rounded-xl shadow-xl overflow-y-auto shadow-black flex-wrap gap-7 md:h-140 h-156 md:py-10 py-5 absolute top-23 xl:right-5 justify-center lg:right-10 md:right-11 right-5 md:p-10 p-1`}>
            { blogs.map(blog => (
                searchTxt.length >= 3 ?
                    <Link onClick={(e) => {e.stopPropagation(); disableBlur(); refreshComponent()}} to={`/blog-details/${blog?._id}`}>
