...
const Login = () => {
    ...
    const handleLogin = async (e) => {
        if (loginData.email.trim() === '' && loginData.password.trim() === '') {
            ...
            try {
                const { data, statusText } = await api.post('auth/login', loginData, {
                    headers: {
    ...
    return (
        <div className='lg:max-w-[1340px] flex flex-col max-w-[560px] w-full mx-auto lg:pl-5 gap-12'>
            ...
            <div className='flex flex-col justify-center items-center gap-[111px] mb-10'>
                <h1 className={`${isDarkModeActive ? "text-white" : "text-[#232536]"} text-[48px] font-bold`}>Login</h1>

                <div className='flex flex-col max-w-[769px] md:w-full h-fit'>
                    <input value={loginData.email} className={`${isDarkModeActive ? "placeholder-white text-white": "placeholder-[#232536] text-[#232536]"} border border-[#6D6E7680] rounded-xs outline-none mb-[33px] py-6 px-6 md:h-fit h-2.5`} placeholder='Enter your email' type="email" onChange={(e) => {
                        handleInput("email", e.target.value)
                    }}/>
                    <input value={loginData.password} className={`${isDarkModeActive ? "placeholder-white text-white": "placeholder-[#232536] text-[#232536]"} border border-[#6D6E7680] rounded-xs outline-none mb-[45px] py-6 px-6 md:h-fit h-2.5`} placeholder='Enter your password' type="password" onChange={(e) => {
